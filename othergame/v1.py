...
if shouldpass:
        ...
        for i in all:
            ...
            if((type(i)!=Player)):
                if 'imgid' in i.__dict__:
                    if i.imgid!=None:
                        if(i.rect.left>0-depth*2):
                            if(i.rect.top>0-depth*2):
                                if(i.rect.left+i.rect.size[0])<940*0+winfo.current_w+depth*2:
                                    if(i.rect.top+i.rect.size[1])<480+depth*2:
    ...
    def main(winstyle=0):
        ...
        #renderthread=threading.Thread(target=updateLoop)
        #renderthread.start()
        #renderLoop()
        while player.alive():
            winfo=pg.display.Info()
